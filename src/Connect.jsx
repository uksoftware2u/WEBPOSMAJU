...
export function ConnectPage({ t, home }) {
  return <main className="connect-page" id="top">
    ...
    <section className="section connect-block"><p className="eyebrow eyebrow--green">Maju Connect</p><h2>{t.overview}</h2><div className="connect-grid">{t.features.map(([title, body], i) => { const Icon = icons[i]; return <article className="connect-card" key={title}><Icon size={32} /><h3>{title}</h3><p>{body}</p></article>; })}</div></section>
